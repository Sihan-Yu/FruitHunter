...
void BaseEngine::StoreObjectInArray(int iIndex, DisplayableObject* pObject)
{
	...
	it = myVectorObject.begin();
	std::advance(it, iIndex);
	myVectorObject.insert(it, pObject);
...
void BaseEngine::DestroyOldObjects()
{
	...
	// clear any existing displayable objects
	myVectorObject.clear();
}


/* Destroy specific object */
void BaseEngine::DestroyObjects(int iIndex)
{
	myVectorObject.erase(myVectorObject.begin() + iIndex);
}
...
void BaseEngine::UpdateAllObjects( int iCurrentTime )
{
	...
	if (!myVectorObject.empty())
	{
		for (int i = 0; myVectorObject[i] != NULL; i++)
		{
			myVectorObject[i]->DoUpdate(iCurrentTime);
...
void BaseEngine::UndrawObjects()
{
	...
	if (!myVectorObject.empty())
	{
		for (int i = 0; myVectorObject[i] != NULL; i++)
		{
			myVectorObject[i]->RedrawBackground();
			if (m_iDrawableObjectsChanged)
				return; // Abort! Something changed in the array
		}
	}
...
void BaseEngine::NotifyAllObjects( int iSignalNumber )
{
	...
	if (!myVectorObject.empty())
	{
		for (int i = 0; myVectorObject[i] != NULL; i++)
		{
			myVectorObject[i]->Notify(iSignalNumber);
		}
	}
...
int BaseEngine::NotifyAllObjectsGetCountNonZero( int iSignalNumber )
{
	...
	if (!myVectorObject.empty())
	{
		for (int i = 0; myVectorObject[i] != NULL; i++)
		{
			if (myVectorObject[i]->Notify(iSignalNumber) != 0)
				iReturn++;
		}
	}
...
int BaseEngine::NotifyAllObjectsGetSum( int iSignalNumber )
{
	...
	if (!myVectorObject.empty())
	{
		for (int i = 0; myVectorObject[i] != NULL; i++)
		{
			iReturn += myVectorObject[i]->Notify(iSignalNumber);
		}
	}
...
int BaseEngine::NotifyAllObjectsGetMax( int iSignalNumber )
{
	...
	if (!myVectorObject.empty())
	{
		for (int i = 0; myVectorObject[i] != NULL; i++)
		{
			int ival = myVectorObject[i]->Notify(iSignalNumber);
			if (ival > iReturn)
				iReturn = ival;
		}
	}
